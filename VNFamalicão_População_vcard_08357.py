    ...
    for a in range(n_anos):
        if a < 10:
            ...
            for i in range(len(data[0]['Dados'][str(Periodo)])):
                Forma = Atividades = data[0]['Dados'][Periodo][i]['dim_4_t']
...
dataf = {'Período de referência dos dados': D1,
            'Local de residência': D2,
            'Duração do casamento anterior': D3,
            'Forma de celebração do casamento a ser dissolvido' : D5,
            'Casamentos dissolvidos por divórcio (Entre pessoas de sexo oposto - N.º)': D4
        }
...
path = r'C:\Users\Andre Pires\OneDrive - LCSD - Associação Data Colab\Documentos\código\SmartCity_Famalicao\integradas\População'
file = 'dataINE_População_' + vcard + '.csv'
